...
func usage(w io.Writer) {
	...
	fmt.Fprintln(w, "  rotor schema [--rbxts]       print a JSON Schema to stdout: rotor.toml by default,")
	...
	fmt.Fprintln(w, "  .rotor/cache/rojo/          Rotor's hashed Rojo resolver cache directory")
	fmt.Fprintln(w, "  *.luau.map                 Luau source maps when compilerOptions.sourceMap is true")
	fmt.Fprintln(w, "  *.rbxtsc.tsbuildinfo       Rotor incremental build state (the .rbxtsc infix is intentional)")
	fmt.Fprintln(w)
	...
	fmt.Fprintln(w, "  UV_THREADPOOL_SIZE         tune the default write-worker count (pool size x 2, max 256)")
